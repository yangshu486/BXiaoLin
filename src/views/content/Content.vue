...
export default {
  ...
  methods: {
    ...
    toClassify(name){
      this.$router.push({path:"/classify",query:{classify:name}})
    }
  },
  mounted() {
    //查询作者名
     this.author.username=localStorage.getItem("username")
    axios({
      method: 'post',
      url: urls(1)+'/findUser',
      data:this.author
      ...
      sessionStorage.setItem("name",this.author.name);
    }),
  
    //查询所有文章
    axios({
      method: 'post',
      url: urls(1)+'/blog/findAllBlog',
      data:this.author 
    }).then((res) => {
